...
열화상 이상탐지 모델(AE, EfficientAD)은 정상 패턴과 다른 영역을 모두 이상으로 잡아내는데, 실제 관심 대상인 "관 형태의 날카로운(sharp) 이상"과 조명·배경 등에서 오는 "뭉툭하고 퍼진(diffuse) 이상"을 구분하지 못하면 오탐이 많아집니다. 이 레포의 핵심 기여는 `scripts/experiments/heat_pipe_shape_heuristic.py` 로, 이상 영역의 **가장자리 선명도(edge sharpness)** 와 **원형도(circularity)** 를 계산해 두 종류를 구분하는 형태 휴리스틱이며, `heat_pipe_shape_sweep.py`로 반경(radius)·강도(delta)를 바꿔가며 합성 데이터에 대해 이 구분이 유효한 범위를 검증합니다.
...
│       ├── heat_pipe_shape_heuristic.py   # 파이프 형태 vs 확산 이상 구분 휴리스틱
